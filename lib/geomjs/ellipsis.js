...
(function() {
  var Ellipsis, Equatable, Formattable, Geometry, Intersections, Path, Point, Surface;
  ...
  Ellipsis = (function() {
    ...
    Ellipsis.prototype.ellipsisFrom = function(radius1, radius2, x, y, rotation, segments) {
      ...
      if (!Math.isFloat(radius1)) {
        radius1 = 1;
      }
      if (!Math.isFloat(radius2)) {
        radius2 = 1;
      }
      if (!Math.isFloat(x)) {
        x = 0;
      ...
      if (!Math.isFloat(rotation)) {
        rotation = 0;
      }
      if (!Math.isFloat(segments)) {
        segments = 36;
      }
